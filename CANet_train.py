...
def train(train_loader, model, optimizer, epoch, save_path):
    ...
    try:
        for i, (images, gts, depth) in enumerate(train_loader, start=1):
            ...
            # bce_iou5 = CE(m1, gts) + iou_loss(m1, gts)
            # bce_iou6 = CE(m2, gts) + iou_loss(m2, gts)
            # bce_iou7 = CE(m3, gts) + iou_loss(m3, gts)
            # bce_iou8 = CE(m4, gts) + iou_loss(m4, gts)
            bce_iou_deep_supervision = (1.6 * bce_iou1 + 1.2 * bce_iou2 + 0.8 * bce_iou3 + 0.4 * bce_iou4)
        ...
        if (epoch) % 5 == 0:
            torch.save(model.state_dict(), save_path + 'CANet_epoch_{}.pth'.format(epoch))
    except KeyboardInterrupt:
        print('Keyboard Interrupt: save model and exit.')
        if not os.path.exists(save_path):
            os.makedirs(save_path)
        torch.save(model.state_dict(), save_path + 'CANet_epoch_{}.pth'.format(epoch + 1))
    ...
    with torch.no_grad():
        ...
        if epoch == 1:
            ...
            if mae < best_mae:
                best_mae = mae
                best_epoch = epoch
                torch.save(model.state_dict(), save_path + 'CANet_epoch_best.pth')
